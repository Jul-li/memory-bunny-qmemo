...
- Home header brand text: `32`, line height `38`, weight `900`, color `#411F0B`.
- Home header prompt text: `18`, line height `22`, weight `900`; with the `4` px gap, the title group height should equal the `64` px avatar.
- Editor title: `22`, weight `900`.
- Section titles: `18`, weight `900`.
- Card titles: `20`, weight `900`.
- Body copy: `15`, weight `600` or `700`, line height around `22`.
...
- Memo cards display Q-style sticker PNGs from `assets/memo-icons/` in the right-side blank area, mapped by memo category: todo uses checklist only, study uses reading, idea uses idea, and life/diary use flower-basket or camera-photo with stable per-memo selection.
...
- Memo card category badges should use matching PNG files from `assets/category-icons/` at `20` when available; fallback to `Ionicons` only when no matching asset exists.
...
- Category filter icons should use matching PNG files from `assets/category-icons/` when available; fallback to `Ionicons` only when no matching asset exists.
...
- Memo form category tags should use matching PNG files from `assets/category-icons/` when available, keep tag height at `40`, and render icons at `24`.
